...
describe('Time Functions', () => {
	describe('CORE', () => {
		...
		test('TIME() type errors', () => {
			expect(() => evaluate('TIME("14", 30, 0)', defaultContext)).toThrow(
				TypeError,
			)
			expect(() => evaluate('TIME(14, "30", 0)', defaultContext)).toThrow(
				TypeError,
	...
	describe('DIFFERENCES', () => {
		...
		test('DIFFERENCE_IN_MINUTES()', () => {
			...
			expect(evaluate('DIFFERENCE_IN_MINUTES(t1, t2)', ctx({ t1, t2 }))).toBe(
				45,
			)
		})
		...
		test('DIFFERENCE_IN_SECONDS()', () => {
			...
			expect(evaluate('DIFFERENCE_IN_SECONDS(t1, t2)', ctx({ t1, t2 }))).toBe(
				30,
			)
		})
		...
		test('DIFFERENCE_IN_HOURS() between PlainDateTimes', () => {
			...
			expect(evaluate('DIFFERENCE_IN_HOURS(dt1, dt2)', ctx({ dt1, dt2 }))).toBe(
				8,
			)
		})
		...
		test('mixed time+datetime rejects', () => {
			...
			expect(() =>
				evaluate('DIFFERENCE_IN_HOURS(t, dt)', ctx({ t, dt })),
			).toThrow(TypeError)
		})
		...
		test('mixed datetime+time rejects', () => {
			...
			const t = new Temporal.PlainTime(10, 0, 0)
			expect(() =>
				evaluate('DIFFERENCE_IN_HOURS(dt, t)', ctx({ dt, t })),
			).toThrow(TypeError)
		})
	})
	...
	describe('COMPARISONS', () => {
		...
		test('IS_SAME_TIME() rejects date', () => {
			const d = new Temporal.PlainDate(2024, 6, 15)
			const t = new Temporal.PlainTime(10, 0, 0)
			expect(() => evaluate('IS_SAME_TIME(d, t)', ctx({ d, t }))).toThrow(
				TypeError,
			)
		})
	})
