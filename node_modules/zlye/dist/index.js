  ...
  static formatValue(value) {
    ...
    if (Array.isArray(value)) {
      if (!value.length)
        return pc.red("[]");
      if (value.length <= 3)
        return pc.red("[") + value.map((v) => this.formatValue(v)).join(", ") + pc.red("]");
    ...
    if (typeof value === "object" && value !== null) {
      ...
      if (!keys.length)
        return pc.red("{}");
      if (keys.length <= 3)
        return pc.red("{") + keys.join(", ") + pc.red("}");
  ...
  static formatCorrectUsage(flagName, value, schema, isNested = false) {
    ...
    if (type === "object") {
      ...
      if (obj._isAnyKeys) {
        ...
        return pc.green(`${prefix}${flagName}.key ${value2}`);
      }
      if (obj._shape) {
        ...
        if (keys.length) {
          ...
          return pc.green(`${prefix}${flagName}.${displayKey} ${this.getExampleValue(obj._shape[key])}`);
        }
      }
    }
    if (type === "union") {
...
${pc.dim("Did you mean")} ${pc.green(similar)}${pc.dim("?")}` : ErrorFormatter.showDiff(isArg ? `${flagName} ${value}` : `--${flagName} ${value}`, isArg ? `${flagName} ${choices[0]}` : `--${flagName} ${choices[0]}`);
...
${ErrorFormatter.showDiff(`${isArg ? "" : "--"}${flagName} "${value}" ${pc.dim(`(${value.length} chars)`)}`, `${isArg ? "" : "--"}${flagName} "${padded}" ${pc.dim(`(${padded.length} chars)`)}`)}`);
    }
...
${ErrorFormatter.showDiff(`${isArg ? "" : "--"}${flagName} "${value}" ${pc.dim(`(${value.length} chars)`)}`, `${isArg ? "" : "--"}${flagName} "${truncated}" ${pc.dim(`(${truncated.length} chars)`)}`)}`);
    }
...
${ErrorFormatter.showDiff(`${isArg ? "" : "--"}${flagName} ${arr.join(",")} ${pc.dim(`(${arr.length} items)`)}`, `${isArg ? "" : "--"}${flagName} ${examples.join(",")} ${pc.dim(`(${examples.length} items)`)}`)}`);
    }
...
${ErrorFormatter.showDiff(`${isArg ? "" : "--"}${flagName} ${arr.join(",")} ${pc.dim(`(${arr.length} items)`)}`, `${isArg ? "" : "--"}${flagName} ${limited.join(",")} ${pc.dim(`(${limited.length} items)`)}`)}`);
    }
    const results = [];
    const errors = [];
    ...
    if (this._shape) {
      ...
      if (unknownKeys.length) {
        ...
        const suggestions = unknownKeys.map((uk) => {
          ...
          return originalSimilar ? `${pc.red(uk)} → ${pc.green(originalSimilar)}` : pc.red(uk);
        });
        const flagName = path.replace("--", "");
          ...
          if (schema._type === "object" && typeof value === "object" && value !== null) {
            ...
            if (providedKeys.length && objSchema._shape) {
              ...
              if (invalidKeys.length && validKeys.length === 0) {
                ...
                throw new CLIError(`${path} has invalid properties: ${pc.red("{" + invalidKeys.join(", ") + "}")}

${pc.dim("Available properties for object format:")}
${originalValidKeys.map((k) => `  ${pc.green(`--${flagName2}.${k}`)} <value>`).join(`
`)}

${pc.dim("Or use other formats:")}
  ${pc.green(`--${flagName2}`)} ${pc.dim("(boolean)")}
  ${pc.green(`--${flagName2} value`)} ${pc.dim("(simple value)")}`);
              }
            }
    ...
    for (const schema of this._schemas) {
      if (schema._type === "object") {
        ...
        if (objSchema._isAnyKeys) {
          examples.push(`${pc.green(`--${flagName}.key value`)} ${pc.dim("(dynamic properties)")}`);
        } else if (objSchema._shape) {
          const keys = Object.keys(objSchema._shape);
          if (keys.length) {
            if (providedKeys.length) {
              examples.push(`${pc.dim("Object properties:")}
${keys.map((k) => `    ${pc.green(`--${flagName}.${k}`)} <value>`).join(`
`)}`);
            } else {
              examples.push(`${pc.green(`--${flagName}.property value`)} ${pc.dim(`(object properties: ${keys.slice(0, 3).join(", ")}${keys.length > 3 ? "..." : ""})`)}`);
            }
          }
        }
...
${pc.dim(pc.blue(`Run with ${pc.bold("--help")} for usage information`))}`);
  ...
  printUsage() {
    if (this.cli._usage) {
      ...
      if (this.cli._positionals.length) {
        parts.push(...this.cli._positionals.map((p) => ("_isVariadic" in p) && p._isVariadic ? pc.dim(`[...${p._name}]`) : pc.dim(`<${p._name}>`)));
      }
  ...
  printCommands() {
    ...
    for (const { name, example, description } of rows) {
      console.log(`  ${pc.cyan(name.padEnd(nameWidth))}  ${example ? pc.dim(example.padEnd(exampleWidth)) : " ".repeat(exampleWidth)}  ${description}`);
    }
    console.log(`  ${pc.cyan("<command> --help".padEnd(nameWidth))}${exampleWidth ? `  ${" ".repeat(exampleWidth)}` : ""}  ${pc.dim("Print help text for command.")}`);
    console.log();
  }
  printOptions() {
    if (!Object.keys(this.cli._options).length)
  ...
  buildCommandUsage(command) {
    ...
    if (command.positionals) {
      parts.push(...command.positionals.map((p) => ("_isVariadic" in p) && p._isVariadic ? pc.dim(`[...${p._name}]`) : pc.dim(`<${p._name}>`)));
    }
    if (Object.keys(command.options).length)
...
${ErrorFormatter.showDiff(`--${key} ${ErrorFormatter.formatValue(rawValue._unionValue)} and --${key}.property`, `--${key} value ${pc.dim("or")} --${key}.property value`)}`);
        }
