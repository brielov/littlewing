...
function expectKind<T extends ASTNode>(node: ASTNode, kind: T["kind"]): asserts node is T {
	expect(node.kind).toBe(kind);
}

// Helper to expect a binary operation with specific operator
function expectBinaryOp(node: ASTNode, operator: Operator): asserts node is BinaryOp {
...
function expectNumber(node: ASTNode, value: number): asserts node is NumberLiteral {
	expectKind<NumberLiteral>(node, NodeKind.NumberLiteral);
	expect(node.value).toBe(value);
}

// Helper to expect a unary operation with specific operator
function expectUnaryOp(node: ASTNode, operator: "-" | "!"): asserts node is UnaryOp {
...
describe("Parser", () => {
	...
	test("if expression error on missing then", () => {
		expect(() => parse("if true 2 else 3")).toThrow('Expected "then" in if expression');
	});

	test("if expression error on missing else", () => {
		expect(() => parse("if true then 2")).toThrow('Expected "else" in if expression');
	});
	...
	test("for expression error on missing in", () => {
		expect(() => parse("for x [1, 2] then x")).toThrow('Expected "in" in for expression');
	});

	test("for expression error on missing then", () => {
		expect(() => parse("for x in [1, 2] x")).toThrow('Expected "then" in for expression');
	});
	...
	test("parse program with semicolons", () => {
		...
		expectKind(node, NodeKind.Program);
		expect((node as { statements: readonly ASTNode[] }).statements.length).toBe(3);
	});
	...
	test("parse boolean in if condition", () => {
		...
		expectKind(node, NodeKind.IfExpression);
		expectKind((node as { condition: ASTNode }).condition, NodeKind.BooleanLiteral);
	});
