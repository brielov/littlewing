...
describe("Date Utils", () => {
	describe("CORE FUNCTIONS", () => {
		...
		test("DATE() with year, month, day", () => {
			const result = evaluate("DATE(2024, 1, 1)", defaultContext) as Temporal.PlainDate;
		...
		test("DATE() handles leap year Feb 29", () => {
			const result = evaluate("DATE(2024, 2, 29)", defaultContext) as Temporal.PlainDate;
			expect(result.year).toBe(2024);
	...
	describe("START/END OF PERIOD", () => {
		test("START_OF_MONTH", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("START_OF_MONTH(d)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.year).toBe(2024);
		...
		test("END_OF_MONTH for June (30 days)", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("END_OF_MONTH(d)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("START_OF_YEAR", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("START_OF_YEAR(d)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.year).toBe(2024);
			expect(result.month).toBe(1);
			expect(result.day).toBe(1);
		});
		...
		test("END_OF_YEAR", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("END_OF_YEAR(d)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.year).toBe(2024);
		...
		test("START_OF_WEEK returns Monday", () => {
			...
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("START_OF_WEEK(d)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("START_OF_QUARTER", () => {
			...
			const result = evaluate("START_OF_QUARTER(d)", dateCtx({ d: may })) as Temporal.PlainDate;
		...
		test("START_OF_QUARTER for Q1", () => {
			...
			const result = evaluate("START_OF_QUARTER(d)", dateCtx({ d: jan })) as Temporal.PlainDate;
			expect(result.month).toBe(1);
			expect(result.day).toBe(1);
		});
	});
	...
	describe("DATE ARITHMETIC", () => {
		test("ADD_DAYS positive", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_DAYS(d, 7)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("ADD_DAYS negative", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_DAYS(d, -5)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.day).toBe(10);
		});
		...
		test("ADD_MONTHS positive", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_MONTHS(d, 2)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("ADD_MONTHS negative", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_MONTHS(d, -3)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.month).toBe(3);
		});

		test("ADD_MONTHS handles month-end overflow", () => {
			const d = new Temporal.PlainDate(2024, 1, 31);
			const result = evaluate("ADD_MONTHS(d, 1)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("ADD_YEARS positive", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_YEARS(d, 5)", dateCtx({ d })) as Temporal.PlainDate;
		...
		test("ADD_YEARS negative", () => {
			const d = new Temporal.PlainDate(2024, 6, 15);
			const result = evaluate("ADD_YEARS(d, -2)", dateCtx({ d })) as Temporal.PlainDate;
			expect(result.year).toBe(2022);
		});

		test("ADD_YEARS from Feb 29 to non-leap year", () => {
			const d = new Temporal.PlainDate(2024, 2, 29);
			const result = evaluate("ADD_YEARS(d, 1)", dateCtx({ d })) as Temporal.PlainDate;
	...
	describe("TIME DIFFERENCES", () => {
		test("DIFFERENCE_IN_DAYS", () => {
			...
			expect(evaluate("DIFFERENCE_IN_DAYS(d1, d2)", dateCtx({ d1, d2 }))).toBe(5);
		});
		...
		test("DIFFERENCE_IN_WEEKS", () => {
			...
			expect(evaluate("DIFFERENCE_IN_WEEKS(d1, d2)", dateCtx({ d1, d2 }))).toBe(3);
		});
		...
		test("DIFFERENCE_IN_MONTHS - exact month boundaries", () => {
			...
			expect(evaluate("DIFFERENCE_IN_MONTHS(d1, d2)", dateCtx({ d1, d2 }))).toBe(3);
		});

		test("DIFFERENCE_IN_MONTHS - day not reached yet", () => {
			const d1 = new Temporal.PlainDate(2024, 1, 15);
			const d2 = new Temporal.PlainDate(2024, 2, 14);
			expect(evaluate("DIFFERENCE_IN_MONTHS(d1, d2)", dateCtx({ d1, d2 }))).toBe(0);
		});
		...
		test("DIFFERENCE_IN_YEARS - exact year boundaries", () => {
			...
			const d2 = new Temporal.PlainDate(2024, 6, 15);
			expect(evaluate("DIFFERENCE_IN_YEARS(d1, d2)", dateCtx({ d1, d2 }))).toBe(4);
		});

		test("DIFFERENCE_IN_YEARS - day not reached yet", () => {
			const d1 = new Temporal.PlainDate(2020, 6, 15);
			const d2 = new Temporal.PlainDate(2024, 6, 14);
			expect(evaluate("DIFFERENCE_IN_YEARS(d1, d2)", dateCtx({ d1, d2 }))).toBe(3);
		});
		...
		test("DIFFERENCE_IN_YEARS - leap year edge case", () => {
			...
			expect(evaluate("DIFFERENCE_IN_YEARS(d1, d2)", dateCtx({ d1, d2 }))).toBe(0);
		});
	...
	describe("INTEGRATION TESTS", () => {
		test("Calculate age in years", () => {
			...
			expect(evaluate("DIFFERENCE_IN_YEARS(birth, today)", dateCtx({ birth, today }))).toBe(34);
		});
		...
		test("Days between two dates", () => {
			...
			expect(evaluate("DIFFERENCE_IN_DAYS(start, end)", dateCtx({ start, end }))).toBe(365);
		});
	...
	describe("PlainDateTime support", () => {
		...
		describe("ARITHMETIC with PlainDateTime", () => {
			test("ADD_DAYS() preserves PlainDateTime type", () => {
				const result = evaluate("ADD_DAYS(dt, 7)", dateCtx({ dt })) as Temporal.PlainDateTime;
			...
			test("ADD_MONTHS() preserves PlainDateTime type", () => {
				const result = evaluate("ADD_MONTHS(dt, 2)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.month).toBe(8);
				expect(result.hour).toBe(14);
			});
			...
			test("ADD_YEARS() preserves PlainDateTime type", () => {
				const result = evaluate("ADD_YEARS(dt, 1)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.year).toBe(2025);
				expect(result.hour).toBe(14);
			});
		...
		describe("DIFFERENCES with PlainDateTime", () => {
			test("DIFFERENCE_IN_DAYS() between PlainDateTimes", () => {
				...
				expect(evaluate("DIFFERENCE_IN_DAYS(dt1, dt2)", dateCtx({ dt1, dt2 }))).toBe(5);
			});
			...
			test("DIFFERENCE_IN_MONTHS() between PlainDateTimes", () => {
				...
				expect(evaluate("DIFFERENCE_IN_MONTHS(dt1, dt2)", dateCtx({ dt1, dt2 }))).toBe(3);
			});
			...
			test("DIFFERENCE_IN_YEARS() between PlainDateTimes", () => {
				...
				expect(evaluate("DIFFERENCE_IN_YEARS(dt1, dt2)", dateCtx({ dt1, dt2 }))).toBe(4);
			});

			test("mixed date+datetime rejects", () => {
				const d = new Temporal.PlainDate(2024, 6, 15);
				expect(() => evaluate("DIFFERENCE_IN_DAYS(d, dt)", dateCtx({ d, dt }))).toThrow(TypeError);
			});

			test("mixed datetime+date rejects", () => {
				const d = new Temporal.PlainDate(2024, 6, 15);
				expect(() => evaluate("DIFFERENCE_IN_DAYS(dt, d)", dateCtx({ dt, d }))).toThrow(TypeError);
			});
		});

		describe("PERIOD BOUNDARIES with PlainDateTime", () => {
			test("START_OF_MONTH() preserves PlainDateTime type", () => {
				const result = evaluate("START_OF_MONTH(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.day).toBe(1);
				expect(result.hour).toBe(14);
			});
			...
			test("END_OF_MONTH() preserves PlainDateTime type", () => {
				const result = evaluate("END_OF_MONTH(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.day).toBe(30);
			});

			test("START_OF_YEAR() preserves PlainDateTime type", () => {
				const result = evaluate("START_OF_YEAR(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.month).toBe(1);
				expect(result.day).toBe(1);
			});
			...
			test("END_OF_YEAR() preserves PlainDateTime type", () => {
				const result = evaluate("END_OF_YEAR(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.month).toBe(12);
				expect(result.day).toBe(31);
			});
			...
			test("START_OF_WEEK() preserves PlainDateTime type", () => {
				...
				const result = evaluate("START_OF_WEEK(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.dayOfWeek).toBe(1);
				expect(result.day).toBe(10);
			});
			...
			test("START_OF_QUARTER() preserves PlainDateTime type", () => {
				const result = evaluate("START_OF_QUARTER(dt)", dateCtx({ dt })) as Temporal.PlainDateTime;
				expect(result).toBeInstanceOf(Temporal.PlainDateTime);
				expect(result.month).toBe(4);
				expect(result.day).toBe(1);
			});
		...
		describe("COMPARISONS with PlainDateTime", () => {
			test("IS_SAME_DAY() same day different time", () => {
				...
				expect(evaluate("IS_SAME_DAY(dt1, dt2)", dateCtx({ dt1, dt2 }))).toBe(true);
			});

			test("IS_SAME_DAY() different days", () => {
				const dt1 = new Temporal.PlainDateTime(2024, 6, 15, 10, 0, 0);
				const dt2 = new Temporal.PlainDateTime(2024, 6, 16, 10, 0, 0);
				expect(evaluate("IS_SAME_DAY(dt1, dt2)", dateCtx({ dt1, dt2 }))).toBe(false);
			});
			...
			test("IS_LEAP_YEAR() with PlainDateTime", () => {
				...
				expect(evaluate("IS_LEAP_YEAR(dt)", dateCtx({ dt: nonLeap }))).toBe(false);
			});
		});
	});
});
