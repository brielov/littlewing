...
describe("Operators", () => {
	describe("Comparison", () => {
		...
		test("cross-type ordered comparison throws TypeError", () => {
			...
			const d = new Temporal.PlainDate(2024, 6, 15);
			expect(() => evaluate("t < d", { variables: { t, d } })).toThrow(TypeError);
			const dt = new Temporal.PlainDateTime(2024, 6, 15, 10, 0, 0);
			expect(() => evaluate("t < dt", { variables: { t, dt } })).toThrow(TypeError);
			expect(() => evaluate("d < dt", { variables: { d, dt } })).toThrow(TypeError);
		});
	...
	describe("Logical", () => {
		...
		test("realistic boolean logic examples", () => {
			...
			expect(evaluate("isStudent = true; age = 20; isStudent || age >= 65")).toBe(true);
			expect(evaluate("isStudent = false; age = 70; isStudent || age >= 65")).toBe(true);
			expect(evaluate("isStudent = false; age = 30; isStudent || age >= 65")).toBe(false);
		});
	});
	...
	describe("If expression", () => {
		...
		test("multi-level conditional logic", () => {
			expect(evaluate("age = 15; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(10);
			expect(evaluate("age = 70; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(8);
			expect(evaluate("age = 30; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(15);
		});
		...
		test("discount calculation with if", () => {
			expect(
				evaluate("price = 100; isPremium = true; price * (1 - (if isPremium then 0.2 else 0))"),
			).toBe(80);
			expect(
				evaluate("price = 100; isPremium = false; price * (1 - (if isPremium then 0.2 else 0))"),
			).toBe(100);
		});

		test("age-based pricing with if", () => {
			expect(evaluate("age = 12; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(10);
			expect(evaluate("age = 70; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(8);
			expect(evaluate("age = 35; if age < 18 then 10 else if age > 65 then 8 else 15")).toBe(15);
		});
	});
});
