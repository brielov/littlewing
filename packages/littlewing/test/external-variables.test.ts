...
describe("External Variables", () => {
	...
	test("realistic pricing with external overrides", () => {
		const pricingScript = `
			basePrice = 100
			quantity = 1
			discount = if quantity > 5 then 0.1 else 0
			shipping = 10
			tax = 0.08
		...
		expect(evaluate(pricingScript, { variables: { quantity: 10 } })).toBeCloseTo(982.8);

		expect(evaluate(pricingScript, { variables: { basePrice: 50, shipping: 5 } })).toBeCloseTo(
			59.4,
		);
	});
