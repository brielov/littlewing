	...
	describe("transform pattern", () => {
		test("transforms AST by doubling all numbers", () => {
			...
			const transformed = visit<ASTNode>(node, {
				...
				PipeExpression: (n, recurse) => pipeExpr(recurse(n.value), n.name, n.args.map(recurse)),
				Placeholder: (n) => n,
			});
	...
	describe("fold pattern", () => {
		test("counts total nodes in AST", () => {
			...
			const count = visit(node, {
				...
				PipeExpression: (n, recurse) =>
					1 + recurse(n.value) + n.args.reduce((sum, a) => sum + recurse(a), 0),
				Placeholder: () => 1,
			});
		...
		test("computes maximum depth of AST", () => {
			...
			const depth = visit(node, {
				...
				PipeExpression: (n, recurse) => 1 + Math.max(recurse(n.value), ...n.args.map(recurse)),
				Placeholder: () => 1,
			});
			...
			visitPartial(
				...
				(n, recurse) => {
					if (n.kind === NodeKind.Program) {
						...
						recurse(n.start);
						recurse(n.end);
					} else if (n.kind === NodeKind.PipeExpression) {
						recurse(n.value);
						n.args.forEach(recurse);
					}
					return undefined;
