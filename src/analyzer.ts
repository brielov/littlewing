...
function containsVariableReference(node: ASTNode): boolean {
	if (isIdentifier(node)) {
		return true // Found a variable reference
	}
	...
	if (isBinaryOp(node)) {
		return (
			containsVariableReference(node.left) ||
			containsVariableReference(node.right)
		)
	}
	if (isUnaryOp(node)) {
	...
	if (isConditionalExpression(node)) {
		return (
			containsVariableReference(node.condition) ||
			containsVariableReference(node.consequent) ||
			containsVariableReference(node.alternate)
		)
	}
	return false
}
